...
class DiffusionModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, device) -> torch.Tensor:
        timestep = torch.randint(1, self.num_timesteps+1, (x.shape[0],)).to(device)
        ...
        # замена self.one_minus_alpha_over_prod на self.sqrt_one_minus_alpha_prod
...
#     def sample_from_noise(self, inputs: torch.Tensor, device) -> torch.Tensor:
...
#             # device
#             eps = self.eps_model(x, torch.tensor(i / self.num_timesteps).repeat(1, 1).to(device))
#             x = self.inv_sqrt_alphas[i] * (x - eps * self.one_minus_alpha_over_prod[i]) + self.sqrt_betas[i] * z
